...
\begin{align} \ref{entropyfunc}
H(X) &= \sum_{i=1}^{n}P(x_i) log(P(x_i)) \label{entropyfunc}
\end{align}

It is typically desired that the entropy of a predictor is large, as this means that the system is producing a large amount of information with each prediction -- corresponding to high-confidence predictions. However, considering an entire batch of predictions, if we are to sum the raw outputs from each example into a single probability distribution as shown in figure \TOCITE{}a, we would hope that each class has a similar total magnitude (assuming the batch contains an identical number of elements per class). A disparity in magnitude would indicate a disparity in entropy of a class (figure \TOCITE{}b). We therefore developed Batch Prediction Entropy (BPE) to give a quantitative measure of class prediction disparity, and to provide an indicator of class exclusivity. \par
BPE is calculated as the entropy of the sum of the raw predictions of an entire batch of examples -- the formula for is given in equation \ref{eq:bpe}, where $\bm{\hat{Y}}$ is a matrix of raw outputs over a whole batch and $B$ is the batch-size. 
\TODO{This is bad notation - get help?}
\begin{align}
BPE(\bm{\hat{Y}}) = H\Bigg(\Big(\sum_{k=1}^{B}\hat{Y}_{k, i}, \Big)\Bigg), i \in [1, batch]
\end{align}
- Trained on CIFAR: \\
- Tried the simpler architecture with head only into meta-learner \\
- Show either one OR the other worked \\
- Show Batch Prediction Entropy (entropy didn't decrease over the batch as it should) \\
- Reasons that it didn't work \\
- Developed BLOCKS \\
