...
We trained a number of Meta-Learner variants on BLOCKS to validate the feasibility of our algorithm and obtained results that indicate successful training. To aide in the stability and speed of training, each experiment was performed using only the source model output head as inputs to the Meta-Learner and used copy-random initialisation. To facilitate this, we utilised two-phase pre-training with the 39 held-out BLOCKS classes. The experiments produced positive results, indicating that the Meta-Learner architecture and algorithm has the capacity to efficiently augment a source model and extend it to a set of previously-unseen classes. Table \ref{tab:blocks-scores} shows the scores for these experiments, where there are minor variations in retention for the differing model output variants. Acquisition, however, indicates that producing gates and deltas for each parameter best aides the Meta-Learner in target class performance. \par

\begin{table}[h] \label{tab:blocks-scores}
	\centering
	\begin{tabular}{|l|l|l|l|l|l|l|}
		\hline
		\begin{tabular}{@{}l@{}}Meta-Learner \\ Inputs\end{tabular}  & Initialisation & 		\begin{tabular}{@{}l@{}}Meta-Learner \\ Outputs\end{tabular} & Source score   & Target score   & Retention      & Acquisition    \\ \hline
	...
	\end{tabular}
	\caption{Results of models trained on BLOCKS; the highest scores in bold.}
\end{table}
...
\begin{table}[h!] \label{tab:omniglot-results}
	...
	\begin{tabular}{|l|l|l|l|l|l|l|}
		\hline
		\begin{tabular}{@{}l@{}}Meta-Learner \\ Inputs\end{tabular}  & Initialisation & 		\begin{tabular}{@{}l@{}}Meta-Learner \\ Outputs\end{tabular} & Source score   & Target score   & Retention      & Acquisition    \\ \hline
		Head                & Copy-Random    & Gates + Deltas       & 0.619          & 0.802          & 0.628          & 0.813          \\ \hline
		Body + Head         & Copy-Random    & Gates + Deltas       & 0.692          & 0.832          & 0.702          & 0.844          \\ \hline
		Body + Head         & Copy-Random    & Deltas               & 0.613          & 0.646          & 0.622          & 0.655          \\ \hline
		Body + Head         & Copy-Random    & Gates + Tanh Deltas  & 0.582          & 0.718          & 0.590          & 0.728          \\ \hline
		Body + Head         & Copy           & Gates + Deltas       & \textbf{0.693} & 0.835          & \textbf{0.703} & 0.846          \\ \hline
		Body + Head         & Rand           & Gates + Deltas       & 0.586          & \textbf{0.863} & 0.594          & \textbf{0.875} \\ \hline
	\end{tabular}
	\caption{Results of models trained on Omniglot; the highest scores in bold.}
\end{table}
