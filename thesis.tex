...
\section{Overview}
\TODO{Should this "overview" section exist? May be better to delete the section name.}
Convolutions have proven useful for computer vision, as local information is sufficient to form an understanding of the relationships between pixel values. We extend that idea to the weights of a network, suggesting that a local view of a network's layers is sufficient to form an understanding of the relationships between weight. Carrying on from this, we build a Meta-Learner which convolves over the weights of a network and its gradients to produce a new set of weights as its output. \par
...
Many variants of the solution are explored in section \ref{approaches}, but we will begin by describing the commonalities between experiments, which form the consistent basis of the solution setup.

\subsubsection{Pre-Training}
Explain how the source models are trained.

\subsubsection{Meta-Learner}
Explain the structure of the meta-learner (basically just inputs and outputs, and that it's embed->encode->decode->unembed)

\subsubsection{Training}
Explain how the meta-learner/target models are trained. Episodes etc.
