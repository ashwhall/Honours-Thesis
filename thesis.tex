...
There are many design factors for the Meta-Learner, which are explored in detail in the following sections. \par
\TODO{Few brief sentences about each approach}
...
\end{align}
These class subsets are grouped into meta-train and meta-test sets with no overlap in classes across the two, with $\sim$20\% in the meta-test set. For our case of 100 classes, this results in 71 meta-train models (classes 1-75) and 20 meta-test models (classes 76-100).\par
Each of these models is trained to convergence on the 5-way classification task, with care taken to not overfit to the relatively small training set - see figure \ref{fig:pre-training-acc:1} for the accuracy during this pre-training phase.
...
\subsection{Meta-Learner} \label{meta-learner-sub}
...
\TODO{\\
	- Figure out the embedding dimensions in/out \\
	- Make a diagram showing that \\
	}

\subsubsection{Encoding/Decoding}
\TODO{\\
	- Make a diagram \\
	- Explain how the outputs can have arbitrary depth (for gates etc) \\
	}
How encoding works \\
Comment on the "... a local view of a network's layers is sufficient to form an understanding of the relationships between weights" \\
Can have a variable number of outputs (gate etc.)

\subsubsection{Unembedding}
How unembedding works (store the original shape) \\
\TODO{\\
	- Refer back to the embedding diagram
	}
...
The training procedure is outlined in figure \ref{fig:ml:2} and algorithm \TOCITE{}, but in its simplest form, consists of the following steps:
\begin{enumerate}
...
\TODO{\\
	- Psuedo code would probably work a treat
	}

\subsection{Variants} \label{approaches}
With such a relatively complex system, there are numerous design choices and opportunities for experimentation. We group these into four categories and explore each separately -- namely, \textit{weight initialisation}, \textit{meta-learner inputs}, \textit{meta-learner outputs} and \textit{dataset complexity}. Here we discuss the significance of each category, and describe the variants explored. \par

\TODO{For the next subsubsections, try to include pseudo-code or diagrams where possible!}

\subsubsection{Weight Initialisation}
What is weight initialisation? \\
Intuition/Why is it important/what happens when you change it? \\
What experiments will be run? \\
Why approached that way? \\
Don't forget Reptile!

\subsubsection{Meta-Learner Inputs}
What is meta-learner inputs? \\
Intuition/Why is it important/what happens when you change it? \\
What experiments will be run? \\
Why approached that way? \\

\subsubsection{Meta-Learner Outputs}
What is meta-learner outputs? \\
Intuition/Why is it important/what happens when you change it? \\
What experiments will be run? \\
Why approached that way? \\

\subsubsection{Dataset Complexity}
What is dataset complexity? \\
Intuition/Why is it important/what happens when you change it? \\
What experiments will be run?\\
Why approached that way? \\
