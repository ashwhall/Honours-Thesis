...
\section{Datasets}
Explain that we pre-train on microimagenet \\
...
As we wish to use this dataset to pre-train the feature extractor of source models, it is helpful for the training images to be of the same dimensions as those the model will eventually work with. Due to this, we created \textit{micro}Imagenet, which is the same as \textit{mini}Imagenet, but with all of the images scaled to 32x32 pixels to match the dimensions of CIFAR100.

\subsection{CIFAR100}
The CIFAR100\parencite{cifar100} dataset consists of 60,000 32x32 images in 100 classes with 600 images per class. 
\subsection{Blocks}
