...
As two-phase pre-training requires additional classes to train the body of the source model, we performed the pre-training as according to the table shown in \ref{tab:pre-train}.\par
...
We begun our experiments by training the Meta-Learner on the CIFAR100 dataset. This proved to be a very difficult task, and yielded unsatisfying results. A recurrent problem encountered was prediction exclusivity, where Meta-Learner ``gets stuck'' producing target models that are only able to provide predictions for either the source classes \textit{or} the target classes. Figure \ref{fig:cifaraccuracy:1} demonstrates this effect by showing the accuracy on the source and target datasets for a number of Meta-Learner variants. As a means of measuring this class exclusivity, we devised the metric \textit{Batch Prediction Entropy}. \par
\begin{figure}[h]
	\centering
...
\subsection{Meta-Learner Outputs}
\subsubsection{Deltas}
In its most basic form, the Meta-Learner can act as a replacement for a standard optimizer, simply updating the weights by adding a delta as shown in equation \ref{delta-rule}. Table \ref{tab:omniglot-results} shows that this gives sub-par performance when compared to the variant with the ability to gate existing weights.
\begin{align} \label{delta-rule}
\bm{\theta}_S + \bm{\Delta}_M
\end{align}
We plotted the scale of the produced deltas with respect to the pre-existing weights as $\bm{\Delta}_M/\bm{\theta}_S$ to gain some understanding of how the two components interact (figure \ref{fig:deltascale:1}). Interestingly, the deltas far outweigh the magnitude of the weights, by an order of magnitude. It is reasonable to expect that the deltas would be in the same order of magnitude as the weights, as this would allow for some of the pre-existing weights to simply be ``passed-through'' as required, as though the Meta-Learner were the identity function. There appears to be no difference in scale between either the deltas that correspond to the source class output weights and those that correspond to the target class output weights. This means that the Meta-Learner is effectively disregarding the existing weights and producing a new set of weights, as the sheer magnitude of the deltas would far outweigh any contribution from the pre-existing weights.

\begin{figure}[h!]
	\centering
	\includegraphics[width=16cm]{deltascale}
	\caption{Delta scale}
	The scale of the produced deltas calculated as $\bm{\Delta}_M/\bm{\theta}_S$
	\label{fig:deltascale:1}
\end{figure}

\subsubsection{Gates and Deltas}
As the Meta-Learner is thus far effectively ignoring the pre-existing weights, we aimed to give it the means to selectively keep or ignore them, by producing a gate value in $[0, 1]$.
\TODO{
	Show the gate distributions
}
\subsubsection{Gates and Tanh Deltas}

\subsection{Weight Initialisation}
As previously discussed, the initialisation of the weights in the target class output head is critical, and ... (do I need an "as previously discussed"?)
\subsubsection{Random Initialisation}
\subsubsection{Copy Initialisation}
\TODO{
	Copy weights acts unexpectedly. We infer that it works well as omniglot wouldn't have crazy sparse weights, features are general across classes, unlike cifar
}
\subsubsection{Copy-Random Initialisation}

\begin{table}[h!] \label{tab:omniglot-results}
	\centering
	\begin{tabular}{|l|l|l|l|l|}
		\hline
		Meta-Learner Inputs & Initialisation & Meta-Learner Outputs & Retention      & Acquisition    \\ \hline
		Head                & Copy-Random    & Gates + Deltas       & 0.628          & 0.813          \\ \hline
		Body + Head         & Copy-Random    & Gates + Deltas       & 0.702          & 0.844          \\ \hline
		Body + Head         & Copy-Random    & Deltas               & 0.622          & 0.655          \\ \hline
		Body + Head         & Copy-Random    & Gates + Tanh Deltas  & 0.590          & 0.728          \\ \hline
		Body + Head         & Copy           & Gates + Deltas       & \textbf{0.703} & 0.846          \\ \hline
		Body + Head         & Rand           & Gates + Deltas       & 0.594          & \textbf{0.875} \\ \hline
	\end{tabular}
	\caption{Retention and acquisition of models trained on Omniglot; the highest scores in bold.}
\end{table}
